...
def calculate_integral_with_error(a, b, tol):
    def f(x): return np.sin(2 * np.cos(x))

    # оценка максимума второй производной (берем с запасом)
    M2 = 6.0  # второго

    # вычисляем необходимое количество разбиений
    h_needed = np.sqrt((24 * tol) / ((b - a) * M2))
    ...
    error_estimate = (b - a)/24 * h**2 * M2

    return integral, error_estimate, n

result, estimated_error, n_used = calculate_integral_with_error(0, np.pi, 0.00001)
